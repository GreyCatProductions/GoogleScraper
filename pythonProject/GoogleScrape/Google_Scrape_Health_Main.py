...
from GoogleScrape.Google_Ranker import get_google_ranking_health
...
def Google(test=False, initial_date = str(datetime.datetime.now().date()), categories = None):
    ...
    try:
        rank_info, all_rank_links = get_google_ranking_health(initial_date, driver=driver, out_path=out_path)
    except Exception as e:
        logging.error(f"google ranking failed {e}")
        driver.close()
        return

    logging.info("google ranking finished.")
    logging.info("starting to load links")
    ...
    logging.info("all links loaded")

    logging.info("getting all apps of category google")
    try:
        all_google_scrape = get_all_apps_of_category_google(categories=categories)
        long_term = pd.read_csv("./data/Google_Health/google_apps_health(17.09.2024).csv")
        google_apps = list(set(all_rank_links + all_google_scrape + diga_links +list(long_term["links"])
        ))
        pd.DataFrame(google_apps, columns=["links"]).to_csv("./data/Google_Health/google_apps.csv", index=False)
        ...
        logging.info(f"loaded all apps of category google. all_rank_links: {len(all_rank_links)} all_scrape: {len(all_google_scrape)} diga links: {len(diga_links)} total: {len(google_apps)}")
    except Exception as e:
        logging.critical(f"google getting all apps of category google failed! {e}")
        driver.close()
        return
    ...
    def get_values(driver):
        ...
        try:
            categories = driver.find_elements(By.XPATH, "//a[contains(@href, '/store/apps/category/')]/preceding-sibling::span")
        ...
        try:
            price = driver.find_element(By.XPATH,
                                        "//button[contains(@aria-label, 'kaufen')]/preceding-sibling::span").text
        except NoSuchElementException:
            try:
                price = driver.find_element(By.XPATH,
                                            "//button[contains(@aria-label, 'Installieren')]/preceding-sibling::span").text
            except NoSuchElementException:
                try:
                    price = driver.find_element(By.XPATH,
                    ...
                    try:
                        ...
                        price = ""
        try:
            description = driver.find_element(By.XPATH, "//meta[contains(@itemprop,'description')]/following-sibling::div[1]").text
        except NoSuchElementException:
            description = ""
        ...
        try:
            ...
            try:
                review_count = driver.find_element(By.XPATH,
                                                   "//h2[text() = 'Bewertungen und Rezensionen']/../../../following-sibling::div/div/div/div/div/div/div[3]").text
            except NoSuchElementException:
                review_count = ""
        ...
        try:
            ...
            try:
                review_average = driver.find_element(By.XPATH,
                                                     "//h2[text() = 'Bewertungen und Rezensionen']/../../../following-sibling::div/div/div/div/div/div/div[1]").text
            except NoSuchElementException:
                review_average = ""
        ...
        try:
            ...
            try:
                updated = driver.find_element(By.XPATH, "//div[contains(text, 'Aktualisiert am')]/following-sibling::div").text
            except NoSuchElementException:
                updated = ""
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    try:
                        email = driver.find_element(By.XPATH, "//div[text()='Support-E-Mail-Adresse']/following-sibling::div").text
                    except NoSuchElementException:
                        email = ""
        try:
            privacy_policy_page = driver.find_element(By.XPATH, "//a[contains(@aria-label,'Datenschutzerklärung')]").get_attribute("href")
        except NoSuchElementException:
            try:
        ...
        out = [driver.current_url, app_name, categories, tags, similar_apps, dev, website, email, price, description, review_count, review_average, review_stars[0], review_stars[1], review_stars[2], review_stars[3], review_stars[4], updated, downloads, usk, privacy_policy_page]
        return out
    ...
    logging.info("Last phase started, starting to scrape")
    for i, link in tqdm(enumerate(google_apps[:how_far_to_scrape]), total=how_far_to_scrape):
        fail_counter = 0
        while True:
            if fail_counter > 10:
                logging.warning("Too many tries in google app. Skipping: " + str(link))
                break
            try:
                driver.get(link)
                if "We're sorry, the requested URL was not found on this server." in driver.page_source:
                    break
                if "Die angeforderte URL wurde auf diesem Server nicht gefunden." in driver.page_source:
                    break
                id = link.split("?")[1]
...
allCategories = ['ANDROID_WEAR', 'ART_AND_DESIGN', 'AUTO_AND_VEHICLES', 'BEAUTY',
                      'BOOKS_AND_REFERENCE', 'BUSINESS', 'COMICS', 'COMMUNICATION', 'DATING',
                      'EDUCATION', 'ENTERTAINMENT', 'EVENTS', 'FINANCE', 'FOOD_AND_DRINK', 'HEALTH_AND_FITNESS',
                      'HOUSE_AND_HOME', 'LIBRARIES_AND_DEMO', 'LIFESTYLE',
                      'MAPS_AND_NAVIGATION', 'MEDICAL', 'MUSIC_AND_AUDIO', 'NEWS_AND_MAGAZINES', 'PARENTING',
                      'PERSONALIZATION', 'PHOTOGRAPHY', 'PRODUCTIVITY',
                      'SHOPPING', 'SOCIAL', 'SPORTS', 'TOOLS', 'TRAVEL_AND_LOCAL', 'VIDEO_PLAYERS', 'WATCH_FACE',
                      'WEATHER', 'GAME', 'GAME_ACTION', 'GAME_ADVENTURE',
                      'GAME_ARCADE', 'GAME_BOARD', 'GAME_CARD', 'GAME_CASINO', 'GAME_CASUAL', 'GAME_EDUCATIONAL',
                      'GAME_MUSIC', 'GAME_PUZZLE', 'GAME_RACING',
                      'GAME_ROLE_PLAYING', 'GAME_SIMULATION', 'GAME_SPORTS', 'GAME_STRATEGY', 'GAME_TRIVIA',
                      'GAME_WORD', 'FAMILY']

medicalOnly = ["MEDICAL", "HEALTH_AND_FITNESS"]
...
def start_google_until_success():
    ...
    while True:
        tries += 1
        try:
            logging.info("Google Health Scrape Started")
            result = Google(test=False, categories=medicalOnly)
            if result == 77:
                logging.info(f"Google function completed successfully! With {tries} tries.")
                send_email(["worstationemailsender@gmail.com"], "Google Scrape Completed!", "Google Scrape Completed!")
                break
        except Exception as e:
...
def hourlyEmail():
    while True:
        time.sleep(3600)
        send_email(["workstationemailsender@gmail.com"], "\n\n".join(log_list), f"Google Health - Hourly Update {len(log_list)} messages")
        log_list.clear()
